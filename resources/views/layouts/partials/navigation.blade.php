<nav-menu inline-template>
    <nav v-click-away="close" @keydown.esc="close" tabindex="0" class="focus:outline-none">
        ...
        <slide-to-left>
            {{-- The "display: none" inline-style prevents an annoying flash before the component is fully rendered. --}}
            <div v-show="isOpen" style="display: none" class="
                    bg-white
                    text-black
                    overflow-y-auto
                    fixed
                    inset-y-0
                    right-0
                    pt-20 xl:pt-screen-20
                    pl-10 xl:pl-screen-10
                    pr-20 xl:pr-screen-20
        ...
        ">
            <div class="
                ...
                sm:space-x-10 xl:space-x-screen-10
            ">
                @foreach(['work', 'about', 'hello'] as $item)
                <button @click="toggle('{{$item}}')" class="
                    font-sans
                    text-sm xl:text-like-sm
                    text-center
                    flex-grow sm:flex-grow-0
                    p-2 sm:p-0
                    h-12 sm:h-8 xl:h-screen-8
                ">
                    {{ __(ucwords($item)) }}
                </button>
                @endforeach
        ...
        ">
            <a class="block" href="/{{ app('translator')->getLocale() }}">
                <svg class="h-8 xl:h-screen-8" viewBox="0 0 92 32" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
