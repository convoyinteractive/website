<nav-menu inline-template>
    <nav v-click-away="close" @keydown.esc="close" tabindex="0" class="focus:outline-none">
        ...
        <slide-to-left>
            {{-- The "display: none" inline-style prevents an annoying flash before the component is fully rendered. --}}
            <div v-show="isOpen" style="display: none" class="bg-white text-black fixed inset-y-0 right-0 pl-10 pr-20 sm:-mr-10 overflow-y-auto pt-20 w-full sm:w-8/12 md:w-6/12 lg:w-4/12">
                <div v-show="isActive('about')" class="h-full p-10">
                ...
                <div v-show="isActive('work')" class="h-full p-10">
                    @foreach($navigation['cases'] as $item)
                    {{ var_dump($item) }}
                    @endforeach
                </div>
                <div v-show="isActive('hello')" class="flex flex-col justify-between h-full p-10">
        ...
        <div class="px-10 pt-10 sm:px-20 fixed inset-x-0 bottom-0 sm:bottom-auto sm:top-0 text-black">
            <div class="mb-5 sm:mb-0 flex items-center rounded-full bg-white sm:bg-transparent shadow sm:shadow-none justify-between sm:justify-end sm:space-x-10">
                @foreach(['work', 'about', 'hello'] as $item)
                <button @click="toggle('{{$item}}')" class="font-sans text-sm xl:text-like-sm text-center flex-grow sm:flex-grow-0 p-2 sm:p-0 h-12 sm:h-8">
                    {{ __(ucwords($item)) }}
                </button>
                @endforeach
            </div>
        ...
        <div class="px-10 pt-10 sm:px-20 fixed top-0 left-0">
