<div class="block w-full h-full bg-gray p-6">
    <span class="block font-bold mb-3">{{ $component->get('label') }}</span>
    @if($component->has('description'))
        <p>{{ $component->get('description') }}</p>
    @endif
    <a href="{{ $component->get('path') }}" class="w-10 h-10 mt-10 bg-white flex justify-center items-center rounded-circle hover:text-white hover:bg-black">
