...
@section('content')
    <div class="{{ $content->styles('hero.wrap') }}">
        <div class="{{ $content->styles('hero.title') }}">
            ...
            <h1 class="font-bold font-sans text-6xl xl:text-like-6xl leading-tight">{{ $content->get('meta.title') }}</h1>
            <span class="block font-sans text-sm xl:text-like-sm">{{ $content->get('meta.subtitle') }}</span>
            @endif
        </div>
